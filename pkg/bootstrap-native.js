...
(function (root, factory) {
  ...
  var Dropdown = function( element) {
    ...
    this.handle = function(e) { // fix some Safari bug with <button>
      ...
      if (target && target.href) (/\#$/g.test(target.href) || /\#$/g.test(target.parentNode.href)) && e.preventDefault();
  
	  for ( var i=0, l = c.length||0; i<l; i++) { l && children.push(c[i]); }
	  if ( target === self.menu || (target.parentNode && target.parentNode === self.menu) || (target.parentNode && target.parentNode.parentNode && target.parentNode.parentNode === self.menu) ) {
		self.toggle(e);
  ...
  //MODAL DEFINITION
  // ===============
  var BSModal = function(element, options) { // element is the is the modal
    options = options || {};
